...
public class RandomEvent extends ScreenAdapter {
...
//-----------------------------other variable--------------------------------
	...
	public RandomEvent(SimCoGame game) {
		this.game = game;
		setStage();
	//-------get random event--------
		Random r = new Random();
		switch(game.simcolife.getCurrPlayer().getCurrentSeason()) {
			case WINTER:
				this.randomE = EventImporter.winterList.get(r.nextInt(EventImporter.winterList.size()));
				break;
			case SUMMER:
				this.randomE = EventImporter.summerList.get(r.nextInt(EventImporter.summerList.size()));
				break;
			case NONE:
				this.randomE = EventImporter.randomList.get(r.nextInt(EventImporter.randomList.size()));
				break;
			default:
				this.randomE = EventImporter.randomList.get(r.nextInt(EventImporter.randomList.size()));
				break;
	...
	public void render(float delta) {
		...
		if(!card.isChecked()) {
			...
			game.batch.draw(currCat, 520, Cards.CARD_Y+260);
		}
		else {
			font.setColor(Color.BLACK);
