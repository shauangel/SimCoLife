...
public class Save extends ScreenAdapter {
...
//-------------------Buttons-----------------
	...
	private ImageButton deleteButton = new ImageButton(new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("save/SaveButtonDeleteImg.png")))), new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("save/SavePressedDeleteImg.png")))));
	private ImageButton[] blockList = new ImageButton[5];
	
	
	public Save(SimCoGame game) {
		this.game = game;
		setStage();
	...
	public void setStage() {
		...
		confirmButton.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				if(RecordImporter.archiveList[choose] == null) {
					RecordImporter.saveRecord(choose, game.simcolife.getCurrPlayer(), SimCoLife.route, SimCoLife.playerNow);
				}
				else {
					final Warning warn = new Warning("", game.skin, "確定要覆蓋檔案嗎？");
					LabelStyle style1 = new LabelStyle(warn.getFont(), Color.RED);
					Label label1 = new Label("確定要覆蓋檔案嗎？", style1);
		...
		//------------------------delete--------------------------
		deleteButton.setPosition(0, 60);
		deleteButton.addListener(new ClickListener() {
			@Override
			public void clicked(InputEvent event, float x, float y) {
				if(RecordImporter.archiveList[choose] == null) {
					RecordImporter.saveRecord(choose, null, null, 0);
				}
				else {
					final Warning warn = new Warning("", game.skin, "確定要刪除檔案嗎？");
					LabelStyle style1 = new LabelStyle(warn.getFont(), Color.RED);
					Label label1 = new Label("確定要刪除檔案嗎？", style1);
					label1.setPosition((500-label1.getWidth())/2, 160);
					ImageButton yes = new ImageButton(new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("DialogButtonYesImg.png")))), new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("DialogPressedYesImg.png")))));
					ImageButton no = new ImageButton(new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("DialogButtonNoImg.png")))), new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("DialogPressedNoImg.png")))));
					yes.setPosition((500-yes.getWidth())/2-100, 55);
